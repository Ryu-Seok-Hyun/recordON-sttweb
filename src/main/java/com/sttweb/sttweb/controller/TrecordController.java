...
public class TrecordController {
  ...
  public ResponseEntity<Map<String,Object>> listAll(
    ...
    // ③ 페이징 조회: 본사(0)/지사관리자(1)/일반(2)
    Page<TrecordDto> paged;
    if ("1".equals(lvl)) {
      // 지사 관리자: 본인 지점의 모든 사용자 내선번호 수집 → 다중 검색
      List<String> nums = memberSvc.listUsersInBranch(
              me.getBranchSeq(), PageRequest.of(0, Integer.MAX_VALUE)
          ).getContent().stream()
          .map(Info::getNumber).filter(Objects::nonNull).toList();
  ...
  public ResponseEntity<ResourceRegion> streamAudio(
    ...
    Info me = requireLogin(authHeader);
    int roleSeq = memberSvc.getRoleSeqOf(me.getMemberSeq());
    if (roleSeq<3) throw new ResponseStatusException(HttpStatus.FORBIDDEN,"청취 권한이 없습니다.");
  ...
  public ResponseEntity<Resource> downloadById(
    ...
    boolean isBranchAdmin  = "1".equals(me.getUserLevel()) && me.getBranchSeq()!=null && me.getBranchSeq().equals(dto.getBranchSeq());
    boolean hasGrant       = dto.getOwnerMemberSeq()!=null && permService.hasLevel(me.getMemberSeq(), dto.getOwnerMemberSeq(),4);
  ...
  public ResponseEntity<Page<TrecordDto>> listByBranch(
    ...
    if (!"0".equals(me.getUserLevel())
        && !("1".equals(me.getUserLevel()) && me.getBranchSeq()!=null && me.getBranchSeq().equals(branchSeq))
    ) {
      throw new ResponseStatusException(HttpStatus.FORBIDDEN,"권한이 없습니다.");
    }
    PageRequest pr = PageRequest.of(page,size);
    return ResponseEntity.ok(recordSvc.findAllByBranch(branchSeq,pr));
  }
}
